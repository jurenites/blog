import {text_bitmap,validate_text_layer,text_locks,toggle_text_lock,draw_text_pixel} from './pixel-text.js?font=4pixel-5';
import {encode_text,grid_from_code,rank_masks,apply_locks,audit_grid,decode_grid,raster_grid,svg_grid,validate_project,ECC_LEVELS} from './core.js?text-font=4pixel-5';
import {append_match,validate_matches,excluded_addresses} from './matches.js?text-font=4pixel-5';
...
const APP_STATE={payload_text:'',version_number:1,error_level:'Q',lock_values:new Array(441).fill(-1),qr_code:null,result_grid:[],tool_name:'paint',text_controls_open:false,text_layer:null,text_drag:null,drag_text_snapshot:null,quarter_turns:0,preview_only:false,alpha_only:false,highlight_index:null,highlight_cells:new Set(),character_rows:[],undo_rows:[],redo_rows:[],cursor_index:null,drag_start:null,drag_indices:new Set(),drag_snapshot:null,search_worker:null,decoded_text:null,found_matches:[],stencil_data:null,decode_timer:null};
...
function snapshot_state(){return {address_pattern:element_by_id('address-pattern').value,payload_text:APP_STATE.payload_text,version_number:APP_STATE.version_number,error_level:APP_STATE.error_level,lock_values:[...APP_STATE.lock_values],text_layer:APP_STATE.text_layer?{...APP_STATE.text_layer,released_cells:(APP_STATE.text_layer.released_cells??[]).map(cell_pair=>[...cell_pair]),drawn_cells:(APP_STATE.text_layer.drawn_cells??[]).map(cell_entry=>[...cell_entry])}:null,alpha_only:APP_STATE.alpha_only,preview_only:APP_STATE.preview_only,mask_index:APP_STATE.qr_code?.mask??-1,quarter_turns:APP_STATE.quarter_turns,protect_structure:element_by_id('protect-structure').checked};}
...
function svg_filename(payload_text){const host_text=payload_text.replace(/^https?:\/\//i,'').split(/[/?#:]/,1)[0];return /^[a-z0-9-]+(?:\.[a-z0-9-]+)+$/i.test(host_text)?`${host_text.toUpperCase()}.svg`:'qr-code.svg';}
function render_matches(){
 ...
 for(const [match_index,match_info] of [...APP_STATE.found_matches.entries()].reverse()){
  ...
  const load_link=document.createElement('a');load_link.href=/^https?:\/\//i.test(match_info.payload_text)?match_info.payload_text:'#qr-canvas';load_link.draggable=false;load_link.className='match-load';load_link.textContent=match_info.payload_text;
  const is_current=match_info.payload_text===APP_STATE.payload_text&&match_info.version_number===APP_STATE.version_number&&match_info.error_level===APP_STATE.error_level&&match_info.mask_index===APP_STATE.qr_code?.mask&&JSON.stringify(match_info.text_layer??null)===JSON.stringify(APP_STATE.text_layer)&&(match_info.quarter_turns??0)===APP_STATE.quarter_turns&&match_info.lock_values.every((cell_value,cell_index)=>cell_value===APP_STATE.lock_values[cell_index]);
  load_link.setAttribute('aria-current',String(is_current));load_link.setAttribute('aria-label',`Load saved match ${match_info.payload_text}`);
  load_link.addEventListener('click',click_event=>{if(match_info.is_active===false){click_event.preventDefault();return;}if(click_event.metaKey||click_event.ctrlKey||click_event.shiftKey||click_event.altKey)return;click_event.preventDefault();const text_selection=window.getSelection();if(text_selection&&!text_selection.isCollapsed&&text_selection.containsNode(load_link,true))return;push_history();restore_snapshot({...match_info,address_pattern:match_info.address_pattern??match_info.payload_text});element_by_id('candidate-text').textContent=match_info.payload_text;verify_current();show_toast('Saved QR and its locked artwork restored.');});
  ...
  export_button.addEventListener('click',()=>{try{const saved_code=encode_text(match_info.payload_text,match_info.version_number,match_info.error_level,match_info.mask_index,match_info.alpha_only===true);const saved_grid=apply_locks(saved_code,text_locks(match_info.lock_values,match_info.text_layer,grid_size,match_info.protect_structure?saved_code.function_grid:null));const is_verified=decode_grid(saved_grid,grid_size)===match_info.payload_text;download_file(svg_grid(rotate_grid(saved_grid,grid_size,match_info.quarter_turns??0),grid_size),svg_filename(match_info.payload_text),'image/svg+xml');if(!is_verified)show_toast('Exported SVG: this saved grid does not decode to the intended text.');}catch(error_info){show_toast(error_info.message);}});
  const remove_button=document.createElement('button');remove_button.type='button';remove_button.className='match-remove';remove_button.textContent='×';remove_button.setAttribute('aria-label',`Delete saved match ${match_info.payload_text}`);remove_button.title='Delete saved match';
  remove_button.addEventListener('click',()=>{APP_STATE.found_matches=APP_STATE.found_matches.filter(saved_match=>saved_match!==match_info);render_matches();save_local();});
  const active_button=document.createElement('button');active_button.type='button';active_button.className='match-active';active_button.setAttribute('role','switch');active_button.setAttribute('aria-label',`Active saved match ${match_info.payload_text}`);active_button.title='Passive matches stay excluded from future searches.';
  function sync_match_state(){const is_active=match_info.is_active!==false;row_element.dataset.active=String(is_active);active_button.setAttribute('aria-checked',String(is_active));active_button.textContent=is_active?'Active':'Passive';detail_row.hidden=!is_active;load_link.setAttribute('aria-disabled',String(!is_active));load_link.title=match_info.payload_text;}
  active_button.addEventListener('click',()=>{match_info.is_active=match_info.is_active===false;sync_match_state();save_local();});
  sync_match_state();
  const header_row=document.createElement('div');header_row.className='match-header';header_row.append(load_link,active_button,remove_button);
  detail_row.append(detail_element,export_button);row_element.append(header_row,detail_row);list_element.append(row_element);
 }
}
...
function apply_tool(cell_index){
 ...
 if(APP_STATE.tool_name==='paint'&&!is_protected){const next_layer=draw_text_pixel(APP_STATE.text_layer,grid_size,cell_index,APP_STATE.result_grid[cell_index]);if(next_layer){APP_STATE.text_layer=next_layer;APP_STATE.result_grid=apply_locks(APP_STATE.qr_code,effective_locks());return true;}}
 if(APP_STATE.tool_name==='lock'&&!is_protected){const next_layer=toggle_text_lock(APP_STATE.text_layer,grid_size,cell_index);if(next_layer){APP_STATE.text_layer=next_layer;APP_STATE.result_grid=apply_locks(APP_STATE.qr_code,effective_locks());return true;}}
 ...
 APP_STATE.lock_values[cell_index]=next_value;APP_STATE.result_grid=apply_locks(APP_STATE.qr_code,effective_locks());return true;
}
...
canvas_element.addEventListener('pointerdown',pointer_event=>{if(pointer_event.button!==0||rotation_animations.length)return;cancel_search();const cell_index=point_to_cell(pointer_event);if(cell_index===null)return;canvas_element.focus({preventScroll:true});canvas_element.setPointerCapture(pointer_event.pointerId);if(APP_STATE.tool_name==='text')begin_text_drag(cell_index);if(APP_STATE.text_drag||APP_STATE.tool_name==='text')return;push_history();APP_STATE.drag_start=cell_index;APP_STATE.drag_snapshot=[...APP_STATE.lock_values];APP_STATE.drag_text_snapshot=APP_STATE.text_layer;APP_STATE.drag_indices=new Set();if(!apply_tool(cell_index))show_toast('QR structure is protected. Turn protection off to experiment.');APP_STATE.drag_indices.add(cell_index);refresh_workspace(false);});
canvas_element.addEventListener('pointermove',pointer_event=>{const cell_index=point_to_cell(pointer_event);APP_STATE.cursor_index=cell_index;cursor_label(cell_index);if(APP_STATE.text_drag){move_text_drag(cell_index);return;}if(APP_STATE.drag_start!==null&&cell_index!==null){if(pointer_event.shiftKey){APP_STATE.text_layer=APP_STATE.drag_text_snapshot;APP_STATE.lock_values=[...APP_STATE.drag_snapshot];APP_STATE.result_grid=apply_locks(APP_STATE.qr_code,effective_locks());const grid_size=APP_STATE.qr_code.size;const first_col=Math.min(APP_STATE.drag_start%grid_size,cell_index%grid_size);const last_col=Math.max(APP_STATE.drag_start%grid_size,cell_index%grid_size);const first_row=Math.min(Math.floor(APP_STATE.drag_start/grid_size),Math.floor(cell_index/grid_size));const last_row=Math.max(Math.floor(APP_STATE.drag_start/grid_size),Math.floor(cell_index/grid_size));APP_STATE.drag_indices=new Set();for(let row_pos=first_row;row_pos<=last_row;row_pos++)for(let col_pos=first_col;col_pos<=last_col;col_pos++){const target_index=row_pos*grid_size+col_pos;apply_tool(target_index);APP_STATE.drag_indices.add(target_index);}}else if(!APP_STATE.drag_indices.has(cell_index)){apply_tool(cell_index);APP_STATE.drag_indices.add(cell_index);}refresh_workspace(false);}else render_canvas();});
function finish_stroke(){if(APP_STATE.text_drag){APP_STATE.text_drag=null;refresh_workspace();return;}if(APP_STATE.drag_start===null)return;APP_STATE.drag_start=null;APP_STATE.drag_snapshot=null;APP_STATE.drag_text_snapshot=null;refresh_workspace();}
...
function choose_tool(tool_name){
 APP_STATE.tool_name=tool_name;if(tool_name==='text')APP_STATE.text_controls_open=true;
 sync_text_controls();render_canvas();
}
function sync_text_controls(){
 const tool_name=APP_STATE.tool_name;canvas_element.dataset.tool=tool_name;element_by_id('pixel-text-controls').hidden=!APP_STATE.text_controls_open;
 document.querySelectorAll('.drawing-toolbar [data-tool]').forEach(button_element=>{
  const is_text=button_element.dataset.tool==='text';
  button_element.setAttribute('aria-pressed',String(is_text?APP_STATE.text_controls_open:button_element.dataset.tool===tool_name));
  if(is_text)button_element.setAttribute('aria-expanded',String(APP_STATE.text_controls_open));
 });
}
document.querySelectorAll('.drawing-toolbar [data-tool]').forEach(button_element=>button_element.addEventListener('click',()=>{
 if(button_element.dataset.tool!=='text'){choose_tool(button_element.dataset.tool);return;}
 cancel_search();push_history();const is_visible=!APP_STATE.text_controls_open;
 if(APP_STATE.text_layer)APP_STATE.text_layer={...APP_STATE.text_layer,is_visible};
 APP_STATE.text_controls_open=is_visible;
 choose_tool(is_visible?'text':APP_STATE.tool_name==='text'?'paint':APP_STATE.tool_name);
 refresh_workspace();
}));
canvas_element.addEventListener('keydown',key_event=>{if(rotation_animations.length){key_event.preventDefault();return;}if(APP_STATE.tool_name==='text'){move_text_key(key_event);return;}const grid_size=APP_STATE.qr_code.size;let cell_index=APP_STATE.cursor_index??Math.floor(grid_size/2)*grid_size+Math.floor(grid_size/2);const key_offsets={ArrowLeft:-1,ArrowRight:1,ArrowUp:-grid_size,ArrowDown:grid_size};if(key_event.key in key_offsets){key_event.preventDefault();const display_index=rotate_index(cell_index,grid_size,APP_STATE.quarter_turns);const next_col=Math.max(0,Math.min(grid_size-1,display_index%grid_size+(key_event.key==='ArrowLeft'?-1:key_event.key==='ArrowRight'?1:0)));const next_row=Math.max(0,Math.min(grid_size-1,Math.floor(display_index/grid_size)+(key_event.key==='ArrowUp'?-1:key_event.key==='ArrowDown'?1:0)));APP_STATE.cursor_index=rotate_index(next_row*grid_size+next_col,grid_size,-APP_STATE.quarter_turns);cursor_label(APP_STATE.cursor_index);render_canvas();}else if(key_event.code==='Space'){key_event.preventDefault();cancel_search();push_history();apply_tool(cell_index);refresh_workspace();}});
...
function start_search(){cancel_search();if(APP_STATE.found_matches.length>=1000){show_toast('This project has reached 1,000 saved matches. Save it before starting a new project.');return;}const pattern_text=element_by_id('address-pattern').value;const alpha_only=!/[a-z]/.test(pattern_text);if(!validate_address_pattern()){element_by_id('generation-message').textContent=element_by_id('pattern-error').textContent;element_by_id('address-pattern').focus();return;}try{if(!has_search_slots(pattern_text))throw new Error('Use ? for the characters that search may choose.');const fitting_patterns=candidate_patterns(pattern_text).filter(candidate_pattern=>{try{encode_text(candidate_pattern.replaceAll('?','A'),APP_STATE.version_number,APP_STATE.error_level,0,!/[a-z]/.test(candidate_pattern));return true;}catch{return false;}});if(!fitting_patterns.length)throw new Error('No matching domain ending fits this QR. Shorten the name or choose a larger grid.');}catch(error_info){show_toast(error_info.message);element_by_id('generation-message').textContent=error_info.message;return;}save_local();const worker_instance=new Worker(new URL('./search-worker.js?encoding=3',import.meta.url));APP_STATE.search_worker=worker_instance;element_by_id('generate-button').disabled=true;element_by_id('find-next').disabled=true;element_by_id('cancel-search').hidden=false;element_by_id('generation-message').textContent='Searching while keeping every locked pixel…';worker_instance.onmessage=({data:message_data})=>{if(APP_STATE.search_worker!==worker_instance)return;if(message_data.type==='progress'){element_by_id('generation-message').textContent=`${message_data.attempt_count.toLocaleString()} candidates checked · ${message_data.phase_name}`;element_by_id('workspace-status').textContent='Searching · painting or changing settings stops the search';}else if(message_data.type==='result'){if(typeof message_data.payload_text!=='string'||!valid_domain_candidate(message_data.payload_text)||message_data.tld_version!==TLD_VERSION){cancel_search();const rejection_text='Rejected an invalid or outdated search result. Your QR was kept. Refresh the page before searching again.';element_by_id('generation-message').textContent=rejection_text;show_toast(rejection_text);return;}if(!candidate_patterns(pattern_text).some(candidate_pattern=>matches_pattern(candidate_pattern,message_data.payload_text))){cancel_search();element_by_id('generation-message').textContent='Rejected a result that changed fixed characters. Your QR was kept.';return;}if(APP_STATE.found_matches.some(match_info=>match_info.payload_text===message_data.payload_text))return;push_history();APP_STATE.preview_only=false;APP_STATE.alpha_only=alpha_only;APP_STATE.payload_text=message_data.payload_text;APP_STATE.qr_code=encode_text(message_data.payload_text,APP_STATE.version_number,APP_STATE.error_level,message_data.mask_index,alpha_only);APP_STATE.result_grid=apply_locks(APP_STATE.qr_code,effective_locks());sync_controls();remember_current_match();refresh_workspace();element_by_id('candidate-text').textContent=message_data.payload_text;element_by_id('generation-message').textContent=`Found a decodable match after ${message_data.attempt_count.toLocaleString()} candidates. Domain registration is unchecked.`;cancel_search();verify_current();}else if(message_data.type==='done'||message_data.type==='error'){cancel_search();element_by_id('generation-message').textContent=message_data.message_text;show_toast(message_data.message_text);}};worker_instance.onerror=()=>{cancel_search();show_toast('Search could not run. Your artwork is preserved.');};worker_instance.postMessage({pattern_text,version_number:APP_STATE.version_number,error_level:APP_STATE.error_level,lock_values:effective_locks(),alphabet_name:element_by_id('search-alphabet').value,time_limit:Number(element_by_id('search-limit').value),protect_structure:element_by_id('protect-structure').checked,excluded_payloads:excluded_addresses(APP_STATE.found_matches)});}
...
function validate_address_pattern(){
 ...
 const invalid_rows=has_search_slots(input_element.value)?invalid_pattern_characters(input_element.value).filter(character_info=>!/[a-z]/.test(character_info.character_text)):[];
 ...
 error_element.textContent=invalid_rows.length?`Unsupported in a search pattern: ${invalid_rows.map(character_info=>`${JSON.stringify(character_info.character_text)} at position ${character_info.character_number}`).join(', ')}. Search accepts A–Z, a–z, 0–9, space or $ % * + - . / : . Complete text without ? can also use other symbols.`:'';
...
function highlight_character(character_index,redraw_canvas=true){
 ...
 if(character_info){
  ...
  const locked_count=character_info.cell_indices.filter(cell_index=>effective_locks()[cell_index]!==-1).length;
...
function encode_live_input(skip_history=false){
 ...
 try{
  ...
  if(has_wildcards){
   const fitting_patterns=candidate_patterns(entered_text).filter(candidate_pattern=>{try{encode_text(candidate_pattern.replaceAll('?','A'),APP_STATE.version_number,APP_STATE.error_level,0,!/[a-z]/.test(candidate_pattern));return true;}catch{return false;}});
  ...
  const alpha_only=has_wildcards&&!/[a-z]/.test(entered_text);
  ...
  const best_result=rank_masks(resolved_text,APP_STATE.version_number,APP_STATE.error_level,effective_locks(),element_by_id('protect-structure').checked,alpha_only);
  if(!skip_history&&(entered_text!==APP_STATE.payload_text||best_result.qr_code.mask!==APP_STATE.qr_code?.mask)){
   push_history();APP_STATE.undo_rows.at(-1).address_pattern=previous_text;
  }
  APP_STATE.preview_only=has_wildcards;APP_STATE.alpha_only=alpha_only;APP_STATE.payload_text=resolved_text;APP_STATE.qr_code=best_result.qr_code;APP_STATE.result_grid=best_result.result_grid;
...
['search-limit','protect-structure'].forEach(element_id=>element_by_id(element_id).addEventListener('input',()=>{cancel_search();refresh_workspace();}));
['qr-version','error-level'].forEach(element_id=>element_by_id(element_id).addEventListener('change',()=>{cancel_search();const next_version=Number(element_by_id('qr-version').value);const next_level=element_by_id('error-level').value;const next_size=17+4*next_version;const old_size=17+4*APP_STATE.version_number;if(next_size<old_size&&APP_STATE.lock_values.some((cell_value,cell_index)=>cell_value!==-1&&(cell_index%old_size>=next_size||Math.floor(cell_index/old_size)>=next_size))){sync_controls();show_toast('Locked artwork extends beyond that grid. Release the out-of-bounds locks first.');return;}try{encode_text(APP_STATE.payload_text,next_version,next_level,-1,APP_STATE.alpha_only);}catch(error_info){sync_controls();show_toast(error_info.message);return;}push_history();const next_locks=new Array(next_size*next_size).fill(-1);for(let row_pos=0;row_pos<Math.min(old_size,next_size);row_pos++)for(let col_pos=0;col_pos<Math.min(old_size,next_size);col_pos++)next_locks[row_pos*next_size+col_pos]=APP_STATE.lock_values[row_pos*old_size+col_pos];APP_STATE.text_layer=validate_text_layer(APP_STATE.text_layer,old_size);APP_STATE.version_number=next_version;APP_STATE.error_level=next_level;APP_STATE.lock_values=next_locks;rebuild_code();if(element_by_id('address-pattern').value&&!matches_pattern(element_by_id('address-pattern').value,APP_STATE.payload_text))encode_live_input(true);}));
...
element_by_id('clear-locks').addEventListener('click',()=>{cancel_search();push_history();APP_STATE.text_layer=null;sync_controls();APP_STATE.lock_values.fill(-1);APP_STATE.result_grid=grid_from_code(APP_STATE.qr_code);refresh_workspace();});
...
element_by_id('export-svg').addEventListener('click',()=>{const payload_result=verify_current();download_file(svg_grid(display_grid(),APP_STATE.qr_code.size),svg_filename(APP_STATE.payload_text),'image/svg+xml');if(payload_result!==APP_STATE.payload_text)show_toast('Exported as experimental: this grid does not decode to the intended text.');});
...
element_by_id('logo-preset').addEventListener('click',async()=>{cancel_search();try{const response_info=await fetch(new URL('./logo.json',import.meta.url));const logo_data=await response_info.json();push_history();APP_STATE.text_layer=null;APP_STATE.version_number=1;APP_STATE.error_level='Q';APP_STATE.quarter_turns=0;APP_STATE.payload_text='HTTPS://1KIC0.FR';APP_STATE.alpha_only=false;APP_STATE.preview_only=false;element_by_id('address-pattern').value=APP_STATE.payload_text;validate_address_pattern();APP_STATE.lock_values=new Array(441).fill(-1);for(let row_pos=0;row_pos<4;row_pos++)for(let col_pos=0;col_pos<20;col_pos++)APP_STATE.lock_values[(row_pos+9)*21+col_pos+1]=logo_data.cells[row_pos*22+col_pos];element_by_id('protect-structure').checked=false;sync_controls();rebuild_code(3);show_toast('Original letters loaded with a one-pixel underline. Structure protection is off for this experiment.');}catch{show_toast('Could not load the logo.');}});
function effective_locks(){return text_locks(APP_STATE.lock_values,APP_STATE.text_layer,17+4*APP_STATE.version_number,element_by_id('protect-structure').checked?APP_STATE.qr_code?.function_grid:null);}
function layer_bitmap(layer_info){return text_bitmap(layer_info.text_value,layer_info.wrap_text?(layer_info.wrap_width??APP_STATE.qr_code.size-2):Infinity);}
function apply_pixel_text(){
 try{
  const grid_size=APP_STATE.qr_code.size;
  const entered_text=element_by_id('pixel-text-input').value;
  if(!entered_text){if(APP_STATE.text_layer){cancel_search();push_history();APP_STATE.text_layer=null;sync_controls();refresh_workspace();}return;}
  const bitmap_info=text_bitmap(entered_text,grid_size-2);
  const next_layer=validate_text_layer({text_value:bitmap_info.text_value,column_pos:Math.max(0,Math.min(APP_STATE.text_layer?.column_pos??Math.floor((grid_size-bitmap_info.width_cells)/2),grid_size-bitmap_info.width_cells)),row_pos:Math.max(0,Math.min(APP_STATE.text_layer?.row_pos??Math.floor((grid_size-bitmap_info.height_cells)/2),grid_size-bitmap_info.height_cells)),white_background:true,outline_size:1,wrap_text:true,wrap_width:grid_size-2,released_cells:APP_STATE.text_layer?.text_value===entered_text?APP_STATE.text_layer.released_cells:[],drawn_cells:APP_STATE.text_layer?.text_value===entered_text?APP_STATE.text_layer.drawn_cells:[],is_enabled:APP_STATE.text_layer?.is_enabled!==false},grid_size);
  cancel_search();push_history();APP_STATE.text_layer=next_layer;sync_controls();choose_tool('text');refresh_workspace();
  element_by_id('pixel-text-help').textContent='Use Text to drag, Draw to change pixels, and Lock to release cells to the QR. Your edits move with the text. Turn text off and on to reset Draw edits. Drag to the grid edge to clip its white border.';
 }catch(error_info){element_by_id('pixel-text-help').textContent=error_info.message+' The previous text layer is kept.';}
}
function begin_text_drag(cell_index){
 const layer_info=APP_STATE.text_layer;if(!layer_info||layer_info.is_enabled===false||layer_info.is_visible===false)return;
 const grid_size=APP_STATE.qr_code.size;const col_pos=cell_index%grid_size;const row_pos=Math.floor(cell_index/grid_size);
 if(col_pos<layer_info.column_pos-1||col_pos>=layer_info.column_pos+layer_bitmap(layer_info).width_cells+1||row_pos<layer_info.row_pos-1||row_pos>=layer_info.row_pos+layer_bitmap(layer_info).height_cells+1)return;
 choose_tool('text');push_history();APP_STATE.text_drag={col_offset:col_pos-layer_info.column_pos,row_offset:row_pos-layer_info.row_pos};
}
function move_text_drag(cell_index){
 if(cell_index===null)return;
 const grid_size=APP_STATE.qr_code.size;const layer_info=APP_STATE.text_layer;
 layer_info.column_pos=Math.max(0,Math.min(Math.max(grid_size-layer_bitmap(layer_info).width_cells,layer_info.column_pos),cell_index%grid_size-APP_STATE.text_drag.col_offset));
 layer_info.row_pos=Math.max(0,Math.min(Math.max(grid_size-layer_bitmap(layer_info).height_cells,layer_info.row_pos),Math.floor(cell_index/grid_size)-APP_STATE.text_drag.row_offset));
 refresh_workspace(false);
}
function move_text_key(key_event){
 if(!APP_STATE.text_layer||APP_STATE.text_layer.is_enabled===false||APP_STATE.text_layer.is_visible===false||!['ArrowLeft','ArrowRight','ArrowUp','ArrowDown'].includes(key_event.key))return;
 key_event.preventDefault();cancel_search();push_history();
 let col_delta=key_event.key==='ArrowLeft'?-1:key_event.key==='ArrowRight'?1:0;
 let row_delta=key_event.key==='ArrowUp'?-1:key_event.key==='ArrowDown'?1:0;
 for(let turn_index=0;turn_index<APP_STATE.quarter_turns;turn_index++)[col_delta,row_delta]=[row_delta,-col_delta];
 const layer_info=APP_STATE.text_layer;const grid_size=APP_STATE.qr_code.size;
 layer_info.column_pos=Math.max(0,Math.min(Math.max(grid_size-layer_bitmap(layer_info).width_cells,layer_info.column_pos),layer_info.column_pos+col_delta));layer_info.row_pos=Math.max(0,Math.min(Math.max(grid_size-layer_bitmap(layer_info).height_cells,layer_info.row_pos),layer_info.row_pos+row_delta));refresh_workspace();
}
function draw_text_outline(cell_size){
 if(!APP_STATE.text_controls_open)return;
 const layer_info=APP_STATE.text_layer;if(!layer_info||layer_info.is_enabled===false||layer_info.is_visible===false)return;
 const grid_size=APP_STATE.qr_code.size;const width_cells=layer_bitmap(layer_info).width_cells;const height_cells=layer_bitmap(layer_info).height_cells;const outline_size=layer_info.outline_size??0;
 // Clip the actual rectangle, including its stroke, rather than drawing a new edge at the crop.
 canvas_context.save();canvas_context.beginPath();canvas_context.rect(4*cell_size,4*cell_size,grid_size*cell_size,grid_size*cell_size);canvas_context.clip();
 canvas_context.translate(4*cell_size,4*cell_size);
 for(let turn_index=0;turn_index<APP_STATE.quarter_turns;turn_index++){canvas_context.translate(grid_size*cell_size,0);canvas_context.rotate(Math.PI/2);}
 canvas_context.strokeStyle=PALETTE.cursor;canvas_context.lineWidth=2;
 canvas_context.strokeRect((layer_info.column_pos-outline_size)*cell_size,(layer_info.row_pos-outline_size)*cell_size,(width_cells+2*outline_size)*cell_size,(height_cells+2*outline_size)*cell_size);
 canvas_context.restore();
}
element_by_id('pixel-text-enabled').addEventListener('change',()=>{if(!APP_STATE.text_layer)return;cancel_search();push_history();const is_enabled=element_by_id('pixel-text-enabled').checked;APP_STATE.text_layer={...APP_STATE.text_layer,is_enabled,drawn_cells:is_enabled?APP_STATE.text_layer.drawn_cells:[]};refresh_workspace();});
element_by_id('pixel-text-input').addEventListener('input',input_event=>{if(!input_event.isComposing)apply_pixel_text();});
element_by_id('pixel-text-input').addEventListener('compositionend',apply_pixel_text);
element_by_id('pixel-text-input').addEventListener('keydown',key_event=>{if(key_event.key==='Enter'){key_event.preventDefault();apply_pixel_text();}});
...
function set_locked_cells(cell_rows){
  ...
  cancel_search();push_history();for(const cell_info of cell_rows)APP_STATE.lock_values[cell_info.row*grid_size+cell_info.column]=cell_info.value===null?-1:cell_info.value;APP_STATE.result_grid=apply_locks(APP_STATE.qr_code,effective_locks());refresh_workspace();verify_current();return inspect_state();
}
