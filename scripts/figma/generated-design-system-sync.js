...
// Source of truth: tokens/tokens.json -> tokens/tokens.flat.json.
...
async function syncVariables() {
  ...
  for (const color of PAYLOAD.colors) {
    ...
    variable.description = 'Synced from tokens/tokens.json. Do not hand-fork in Figma.';
  }
...
async function createColorSection(page, x, y) {
  ...
  section.appendChild(await textNode('Strict monochrome palette and semantic aliases from tokens/tokens.json.', 14, PAYLOAD.values.muted, 400));
...
async function main() {
  ...
  header.appendChild(await textNode('Synced from tokens/tokens.json. Update tokens first, then regenerate this page.', 15, PAYLOAD.values.muted, 400));
  ...
  figma.notify('Synced Blog Jurenites tokens, typography frames, and UI component frames.');
  return {
...
(async () => {
  try {
    ...
    console.error(error);
    figma.notify('Blog Jurenites Figma sync failed. Check the plugin console.');
    throw error;
  }
})();
