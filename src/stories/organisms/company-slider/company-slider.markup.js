  ...
  return `<section class="company-slider" data-company-slider aria-label="${escape_html(section_heading)}" aria-roledescription="carousel">
    ...
    <ul class="company-slider__track horizontal-scrollbar" data-company-track tabindex="0" aria-label="Companies. Use left and right arrow keys to scroll.">
      ${company_items.map((company_data) => `<li class="company-slider__item">
        <a id="company-${escape_html(company_data.company_key)}" class="company-slider__company-link" href="${escape_html(company_data.website_url)}" rel="external" aria-label="${escape_html(company_data.company_name)}">
          ...
          <span class="company-slider__name"><span>${escape_html(company_data.company_name)}</span>${icon_markup({ icon_name: 'external-link', class_name: 'company-slider__external-mark' })}</span>
        </a>
