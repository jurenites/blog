import input_text_template from "./input-text.template.html?raw";
import input_text_group_template from "./input-text-group.template.html?raw";
...
function required_indicator_markup(is_required) {
  return is_required ? '<span class="input-text__required" aria-hidden="true">&#x20;*</span>' : "";
}
...
function description_markup(field_id, field_description) {
  ...
  return `<p class="input-text__description" id="${escape_html(field_id)}-description">${escape_html(field_description)}</p>`;
}
...
function validation_error_markup(field_id, validation_state, validation_message) {
  ...
  return `<p class="input-text__error" id="${escape_html(field_id)}-error" role="alert">${escape_html(validation_message)}</p>`;
}
...
function standard_control_markup({
  ...
  if (field_control === "textarea") {
    return `<textarea class="input-text__control input-text__control--textarea" id="${escaped_id}" name="${escaped_name}" rows="4" placeholder="${escaped_placeholder}" ${control_attributes}></textarea>`;
  }
  ...
  if (field_control === "select") {
    return select_input_markup({
      ...
      native_class_names: "input-text__control input-text__control--select",
      accessibility_attributes: control_attributes,
    });
  }

  if (field_control === "file-upload") {
    return `<input class="input-text__file" id="${escaped_id}" name="${escaped_name}" type="file" ${control_attributes}>`;
  }
  ...
  return `<input class="input-text__control text-input__control" id="${escaped_id}" name="${escaped_name}" type="${input_type}" autocomplete="${autocomplete_value}" placeholder="${escaped_placeholder}" ${control_attributes}>`;
}
  ...
  return option_value_list(choice_options)
    .map((choice_option) => {
      ...
      const label_class_name = field_control === "choice-chips"
        ? "input-text__chip-label"
        : "input-text__choice-label";

      return `<div class="input-text__choice"><input class="${input_class_name}" id="${escape_html(choice_id)}" name="${escape_html(control_name)}" type="${input_type}" value="${escape_html(choice_value)}" ${choice_attributes}${checked_attribute}><label class="${label_class_name}" for="${escape_html(choice_id)}">${escape_html(choice_option)}</label></div>`;
    })
    .join("");
}

export function input_text_markup({
  field_data_type,
  field_control,
  field_id,
  field_name,
  ...
  const error_class_name = validation_state === "error" ? " input-text--error" : "";
  const disabled_class_name = is_disabled ? " input-text--disabled" : "";
  const input_text_classes = `${error_class_name}${disabled_class_name}`;

  if (GROUP_CONTROL_NAMES.has(field_control)) {
    return render_template(input_text_group_template, {
  ...
  if (field_control === "single-checkbox") {
    return render_template(input_text_template, {
      ...
      field_error_markup,
      field_label_markup: "",
      input_text_classes: `${input_text_classes} input-text--single-choice`,
    });
  }
  ...
  return render_template(input_text_template, {
    ...
    field_data_type: escape_html(field_data_type),
    field_error_markup,
    field_label_markup: `<label class="input-text__label" for="${escape_html(field_id)}">${escape_html(field_label)}${required_indicator_markup(is_required)}</label>`,
    input_text_classes,
  });
}
