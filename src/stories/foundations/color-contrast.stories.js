  ...
  return Object.entries(token_map)
    .filter(
      ([token_name, token_data]) =>
        token_data.type === "color" && token_name.startsWith("color-palette-") && token_data.path[1] === "palette",
    )
    .map(([token_name, token_data]) => ({
      token_name,
      token_label: token_name.replace("color-palette-", ""),
      token_value: token_data.css,
    }));
}
...
function body_cell_markup(text_token, background_token) {
  ...
  return `<td class="contrast-cell" style="background: ${escape_html(background_token.token_value)}; color: ${escape_html(
    text_token.token_value,
  )};">
